...
static void sched_core_cgroup_online(struct task_group *parent, struct task_group *tg)
{
...
static void sched_core_cgroup_free(struct task_group *tg)
{
	sched_core_put_cookie(tg->core_cookie);
}

static unsigned long sched_core_cgroup_cookie(struct task_group *tg)
{
...
#ifdef CONFIG_SCHED_CORE
static u64 cpu_sched_core_read_u64(struct cgroup_subsys_state *css, struct cftype *cft)
{
	return !!css_tg(css)->core_cookie;
}

static int cpu_sched_core_write_u64(struct cgroup_subsys_state *css, struct cftype *cft, u64 val)
{
